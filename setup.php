<?php
/**
 * Created by PhpStorm.
 * User: Andrew Quaye
 * Date: 15/10/2018
 * Time: 8:20 AM
 */
$rave = array(
    'LivePayment' =>"https://api.ravepay.co/flwv3-pug/getpaidx/api/v2/hosted/pay",
    'TestPayment' =>"https://ravesandboxapi.flutterwave.com/flwv3-pug/getpaidx/api/v2/hosted/pay",

    'LivePaymentVerify'=>"https://api.ravepay.co/flwv3-pug/getpaidx/api/v2/verify",
    'TestPaymentVerify'=>"https://ravesandboxapi.flutterwave.com/flwv3-pug/getpaidx/api/v2/verify",

    'LiveSECKEY'=>"FLWSECK-45a0c821f1e1a352a1a45f4fd1c727ca-X",
    'TestSECKEY'=>"FLWSECK-45a0c821f1e1a352a1a45f4fd1c727ca-X"
);

$exchange_rate =array("usd" => '0.19',"ghs" => '1',"ngn" => '66.42');



